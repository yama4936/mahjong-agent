...
force-autoの公開局面認識は自手番の経路へ入れず、他家手番中に別の常駐ハイブリッド
認識プロセスで更新します。ドラ、各家の河、リーチ表示、副露を最大15秒のキャッシュ
...
`FORCE_AUTO_PUBLIC_CACHE_MAX_AGE_MS`で変更でき、`--no-public-cache`で無効化できます。
...
他家手番のバックグラウンドだけで走るため、自手番の700ms期限には加算されません。
