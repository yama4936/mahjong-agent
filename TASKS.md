...
- [x] `done` Fix the remaining 5+20 multi-meld retry loop. In the first 5+20 match, after automatic calls the deployed operator repeatedly proposed x=1446.5 and aborted it as outside current hand geometry from 14:13:23+09 through at least 14:14:57, including after another pon (`artifacts/friend-5-20-two-meld-failure.png`, evidence frame `artifacts/friend-5-20/frames/2026-09-21T05-14-56.898415+00-00.jpg`). The known-count proposal constraint did not prevent this live candidate; make the real recognition result/click target use the compact concealed row and shifted draw slot, then prove recovery without waiting for timeout.
- [x] `done` Eliminate transient off-row candidates after three open melds in 5+20. After the third automatic pon in South 2, the operator proposed and safely rejected board/opponent coordinates x=1047.5,y=643.5, x=1730.5,y=617.5, and x=1761.5,y=715 before later recovering (`artifacts/friend-5-20/frames/2026-09-21T05-24-26.726820+00-00.jpg` through `05-24-46.615576+00-00.jpg`). Constrain the 5-tile three-meld hand proposal and retry path to the calibrated bottom concealed row/shifted draw region so no out-of-bounds candidate is produced even transiently.
- [x] `done` Suppress all discard evaluation after a winning action until the round result is observed. In the second 5+20 match, automatic tsumo succeeded at 14:41:01+09, but the operator immediately resumed discard attempts during the win animation and logged several out-of-bounds/changed-hand aborts before `outcome_attached` at 14:41:09. Treat ron/tsumo clicks as a round-terminal latch, clear it only on a verified new round, and add a live-frame/event-sequence regression.
- [x] `done` Restore a live open hand after a process restart in 5+20. After restarting for the win-terminal-latch deployment, the operator logged `compact hand appeared before any observed call` continuously from 14:46:29+09 to 14:46:52, timed out, and required away-mode recovery. Rebuild the confirmed meld count from stable bottom-hand geometry plus calibrated own-meld evidence (or persist it durably), reject ambiguous frames, and prove the restarted process acts before the 5-second base clock expires.
...
- [x] `done` Eliminate the long post-pon hand-change retry stall in 5+20. In the third 5+20 match, automatic pon succeeded at 15:08:00+09, then ten consecutive pre-click checks aborted from 15:08:04 through 15:08:22 with `hand changed since evaluation` pixel deltas between 1.586 and 5.963 before play finally resumed at 15:08:24 (`artifacts/friend-5-20/python-operator.jsonl`). Compare equivalent stable regions after call animation, preserve stale-hand protection, and add a regression proving the required discard is sent within the 5-second base clock.
